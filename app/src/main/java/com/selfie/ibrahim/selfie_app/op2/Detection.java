package com.selfie.ibrahim.selfie_app.op2;
...
public class Detection {
    ...
    public Mat drawInfo(Mat finalImg) {
        ...
        if ((fingerPts != null) && (handCenterPoint != null)) {
            ...
            Imgproc.circle(finalImg, handCenterPoint, 9, new Scalar(0), 4);
            Imgproc.circle(finalImg, handCenterPoint, (int) (radius * 1.4), new Scalar(0), 4);

            for (int i = 0; i < fingerPts.length; i++) {
                if (fingerPts[i] != null) {
                    try {
                        Imgproc.putText(finalImg, fingerPts[i].getFingerName(), fingerPts[i], Core.FONT_HERSHEY_SIMPLEX,
                                0.5, new Scalar(255, 124, 113, 0));
                        Imgproc.circle(finalImg, fingerPts[i], 4, new Scalar(255, 124, 113, 0), 4);

                        Imgproc.line(finalImg, handCenterPoint, fingerPts[i], new Scalar(0), 4);
                    } catch (Exception e) {
